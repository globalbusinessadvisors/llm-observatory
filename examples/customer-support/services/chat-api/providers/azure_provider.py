...
from openai import AsyncAzureOpenAI, RateLimitError as OpenAIRateLimitError
...
class AzureOpenAIProvider(BaseProvider):
    ...
    # Pricing per 1M tokens (similar to OpenAI, may vary by region)
    PRICING = {
    ...
    async def complete(
        ...
        **kwargs
        ...
        try:
            ...
            raise AuthenticationError(f"Azure OpenAI authentication failed: {e}")
    ...
    def estimate_cost(self, prompt_tokens: int, completion_tokens: int, model: str) -> float:
        ...
        # Normalize Azure model names to pricing keys
        pricing_key = model.replace("gpt-", "gpt-").replace("35", "3.5")
        pricing = self.PRICING.get(pricing_key, {"prompt": 10.0, "completion": 30.0})
